...
export function AboutSection() {
  return (
    <section className="relative mx-auto grid max-w-7xl gap-14 px-6 py-24 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
      ...
      <Reveal className="relative" delay={0.04}>
        <div className="group relative aspect-[4/5] overflow-hidden rounded-[36px] shadow-2xl">
          ...
          <div className="absolute inset-0 bg-cover bg-center transition duration-700 group-hover:scale-[1.04]" style={{ backgroundImage: `url('${featureImage}')` }} />
        ...
        <div className="absolute -bottom-10 left-4 right-4 grid grid-cols-2 gap-3 md:left-6 md:right-auto md:w-[420px]">
          <Reveal delay={0.18}>
            <div className="rounded-2xl border border-[#E6E2D9] bg-white p-4 shadow-xl">
              ...
              <div className="text-xs uppercase tracking-[0.22em] text-slate-500">Hubs</div>
            </div>
          </Reveal>
          <Reveal delay={0.24}>
            <div className="rounded-2xl border border-[#E6E2D9] bg-white p-4 shadow-xl">
              <div className="text-2xl font-semibold text-[#043E52]">35k+</div>
              <div className="text-xs uppercase tracking-[0.22em] text-slate-500">Youth reached</div>
            </div>
          </Reveal>
        </div>
      </Reveal>
      ...
      <Reveal className="lg:pl-8" delay={0.08}>
        <div className="inline-flex items-center gap-2 rounded-full border border-[#E16A3D]/20 bg-[#E16A3D]/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.28em] text-[#E16A3D]">
        ...
        <StaggerGrid className="mt-6 grid gap-3 sm:grid-cols-2" delay={0.12}>
          {featureBullets.map((item) => (
            <StaggerItem key={item}>
              <div className="flex items-center gap-3 rounded-xl bg-white px-4 py-3 shadow-sm ring-1 ring-slate-200/70">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#016A6D]/10 text-[#016A6D]">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-5">
          ...
          <div className="max-w-sm text-sm leading-7 text-slate-500">
            Built to feel warm, credible, youth-centered, and easy to use across devices.
          </div>
        </div>
      </Reveal>
    </section>
